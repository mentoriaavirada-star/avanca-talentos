...
async function updatePDI(body: any) {
  ...
  const lines = ["", "=== AVALIAÇÃO FINAL ===", "Data: " + new Date().toLocaleDateString("pt-BR"), "STATUS: Concluído", "",
    ...comps.map((c: any) => "- " + c.name + " | Antes: " + c.nivelAntes + "/5 → Depois: " + c.nivelDepois + "/5" + (c.obsDepois ? " | Obs: " + c.obsDepois : ""))];
  await notion.blocks.children.append({ block_id: pageId, children: lines.map((t) => paragraph(t)) });
  return { ok: true };
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    if (action === "create") {
      return NextResponse.json(await createPDI(body));
    }
    if (action === "list") {
      return NextResponse.json(await listPDIs(body.gestorEmail));
    }
    if (action === "get") {
      return NextResponse.json(await getPDI(body.pageId));
    }
    if (action === "update") {
      return NextResponse.json(await updatePDI(body));
    }
